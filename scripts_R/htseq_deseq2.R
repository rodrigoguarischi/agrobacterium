...
Y1XY3 = results(dds, lfcThreshold = 1, contrast = c("condition","Y3","Y1"))
Y1XY3 = Y1XY3[order(rownames(Y1XY2)),] #ordenar pelo nome do gene

Y1XF1 = results(dds, lfcThreshold = 1, contrast = c("condition","F1","Y1"))
Y1XF1 = Y1XF1[order(rownames(Y1XY2)),] #ordenar pelo nome do gene

Y1XF2 = results(dds, lfcThreshold = 1, contrast = c("condition","F2","Y1"))
Y1XF2 = Y1XF2[order(rownames(Y1XY2)),] #ordenar pelo nome do gene

Y1XF3 = results(dds, lfcThreshold = 1, contrast = c("condition","F3","Y1"))
...
colnames(lfc_DEG) = c("Y1","Y2","Y3","F1","F2","F3")
...
write.table(as.data.frame(Y1XF1_out), file = "Y1vsF1.txt", sep = "\t", row.names = TRUE, col.names = TRUE, na = "NA", dec = ".")

Y2XF2_out = subset(Y2XF2, Y2XF2$padj <= 0.05)
write.table(as.data.frame(Y2XF2_out), file = "Y2XF2.txt", sep = "\t", row.names = TRUE, col.names = TRUE, na = "NA", dec = ".")

Y3XF3_out = subset(Y3XF3, Y3XF3$padj <= 0.05)
write.table(as.data.frame(Y3XF3_out), file = "Y3XF3.txt", sep = "\t", row.names = TRUE, col.names = TRUE, na = "NA", dec = ".")

Y1XY2_out = subset(Y1XY2, Y1XY2$padj <= 0.05)
write.table(as.data.frame(Y1XY2_out), file = "Y1XY2.txt", sep = "\t", row.names = TRUE, col.names = TRUE, na = "NA", dec = ".")

Y2XY3_out = subset(Y2XY3, Y2XY3$padj <= 0.05)
write.table(as.data.frame(Y2XY3_out), file = "Y2XY3.txt", sep = "\t", row.names = TRUE, col.names = TRUE, na = "NA", dec = ".")

F1XF2_out = subset(F1XF2, F1XF2$padj <= 0.05)
write.table(as.data.frame(F1XF2_out), file = "F1XF2.txt", sep = "\t", row.names = TRUE, col.names = TRUE, na = "NA", dec = ".")

F2XF3_out = subset(F2XF3, F2XF3$padj <= 0.05)
write.table(as.data.frame(F2XF3_out), file = "F2XF3.txt", sep = "\t", row.names = TRUE, col.names = TRUE, na = "NA", dec = ".")
