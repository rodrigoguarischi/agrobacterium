...
de$results.output$human.homologues  <- add.info(ensemblIDs = de$results.output$ensembl.ids, field.name = "hsapiens_homolog_associated_gene_name", mart = mouse.mart );
...
pubmed.results <- sapply(
  paste( de$results.output$gene.symbol, de$results.output$human.homologues, sep = "; " ),
...
# Load drugbank info
for( file in list.files(path = "aux_files/drugbank_13.01.2017", pattern = "(all|pharmacologically_active).csv", recursive = TRUE, full.names = TRUE) ){
...
for( drug.group in names(drugbank$all) ){
  ...
  de$results.output[[paste0(drug.group,".drugs")]] <- sapply( 
    strsplit( paste( de$results.output$gene.symbol, de$results.output$human.homologues, sep = "; " ), "; "), 
...
write.csv( de$results.output, file = "~/Desktop/de_results.csv" )

# Write background file (genes with at least 10 reads in 2 distinct samples)
write.csv(
  x = counts(tmp$dds)[ rowSums( counts(tmp$dds) >= 10 ) >= 2, ],
  file = "~/Desktop/background.csv",
  row.names = TRUE
  );

### Step 4 - Save Rdata object for later use
save( list = c("de", "ddsFull", "drugbank", "pubmed.results"), file = "aux_files/expression_data.Rdata" );
